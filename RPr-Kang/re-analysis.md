...
Created 2021-10-28 | Last Updated 2021-11-4
...
One of the biggest challenges of this notebook to be used for as a guiding material for those who wish to learn about enhanced two-step floating catchment area method is that the function used to explain this method is complex and poorly commented. While attempting to understand the python code is has already been a steep learning curve many, these complicated functions further add to the incomprehensibility of the notebook. To be more specific, the major function that construct catchment area and calculates accessibility is integrated into the parallel processing function, and we never get to see how its output look like.

Thus, besides revising the original method, the major function that implements catchment area calculation has been broken into several sub-functions (with slight modifications) to allow for a better understanding of the output of each segment of the function. To achieve so, we took the first hospital of the hospital data set and use it as input to the function.
...
![icu](icu.jpg)
*Figure 2: Spatial accessibility of ICU beds in Chicago obtained from original analysis (left) and reanalysis (right)*
